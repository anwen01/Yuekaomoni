...
public class Fragmeent2 extends Fragment {
    ...
    public void getData() {
        OkHttp3Utils.getInstance().doGet("http://www.yulin520.com/a2a/impressApi/news/mergeList?sign=C7548DE604BCB8A17592EFB9006F9265&pageSize=20&gender=2&ts=1871746850&page=1", new GsonObjectCallback<Bean>() {
            ...
            public void onUi(Bean bean) {
                ...
                for (int i = 0; i < 3; i++) {
                    List<ChildBean> clist1 = new ArrayList<ChildBean>();
                    for (int j = 0; j < 3; j++) {
                        clist1.add(new ChildBean(data.get(j).getTitle(), "100", data.get(j).getUserImg(), false, 1));
                    }
                    slist.add(clist1);
                }
